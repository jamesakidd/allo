...
- [x] Move a task to another list (2026-09-24): a List field in the task's edit sheet, shown when there's more than one list
  - It's just an edit to the task's list, part of the content group, so it syncs like any other edit and keeps priority, due date and done state. Ticking a task off on one phone never collides with moving it on another; a move and a title edit on the same task, both offline, is last-to-sync-wins like the grocery list
  - Moving a task off the screen looks exactly like deleting it, so a snackbar says "Moved to Garden" with **Undo**. Undo moves it back by id, so a sync in between can't make it act on a stale copy, and it does nothing if the original list has since been deleted
  - **Both orders of the delete race are handled.** List deleted first, move second: the server refuses a task on a deleted list and hands back its version, so the task stays where it was. Move first, delete second: the deleting phone never saw the moved task, so the server retires any tasks still on a list when it's deleted — otherwise the task would sit on a deleted list, stored and never shown. The same two guards cover adding a task. Proven by switching each guard off and watching its test fail
- [x] Move several tasks at once (2026-09-24): long-press a task, or "Select tasks" in the ✎ menu. Only offered when there's another list to move to
  - In selection mode the **whole row** toggles selection and selected rows are highlighted blue. The checkbox keeps its meaning — it still shows done or not — but can't be ticked while selecting, so selecting a task by its left edge never marks it done by accident. (That design was James's call, and better than mine, which repurposed the checkbox.)
