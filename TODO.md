...
- [x] Several task lists from the start (House, Garden, Errands…). The picker at the top of Tasks always shows, with "+ New list…" at the bottom of it and an edit menu beside it for rename and delete. A new list becomes the current one; the last list cannot be deleted, since the screen would have nowhere to put anything
  ...
  - Seen once in use and **not reproduced**: on first load the picker showed a list's GUID instead of its name. The stored name was intact ("Tasks"), a cold load reads correctly, and a load where the names arrive late reads blank then the name. The GUID only lives in a `type=hidden` input. If it recurs, note what was open and how the page was reached
